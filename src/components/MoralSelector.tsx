...
const MORAL_CATEGORIES: MoralCategory[] = [
  ...
  },
  {
    ...
    emoji: "❤️",
    morals: ["Radość z dzielenia się 🤝", "Oswojenie złości 🦖", "Moc słowa Przepraszam 🌸", "Empatia i pomaganie 🫂"],
  },
  {
    name: "Mądre Wartości",
    emoji: "💎",
    morals: [
      ...
      "Ciekawość świata 🌍",
      "Uczciwość i prawda 💡",
      "Higiena cyfrowa (oddech od ekranu) 📵",
    ],
  },
];
...
const MoralSelector = ({ value, onChange }: MoralSelectorProps) => {
  ...
  const isSelected = (category: string, moral: string) => value?.category === category && value?.moral === moral;
      ...
      {MORAL_CATEGORIES.map((cat) => (
        <div key={cat.name} className="rounded-xl overflow-hidden">
          ...
          >
            ...
            {value?.category === cat.name && !randomMode && (
              <span className="text-xs text-accent truncate max-w-[120px]">{value.moral}</span>
            )}
            <ChevronDown
